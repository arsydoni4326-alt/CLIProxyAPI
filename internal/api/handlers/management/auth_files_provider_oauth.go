...
func (h *Handler) requestKimiTokenWithDomain(c *gin.Context, domain string) {
	...
	// Initialize Kimi auth service for the selected domain (with an optional direct-connection override)
	kimiAuth := kimi.NewKimiAuthWithDomainAndProxyURL(h.cfg, domain, oauthProxyOverride(c))
